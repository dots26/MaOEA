...
#' \code{referencePoint} NSGA-III require a set of reference points defined a priori. The reference can be any point. If not supplied, 5*nObjective points are generated from a sobol sequence. Size: nrow = nObjective,ncol = number of refernce points
...
NSGA3.test <- function(population,fun="DTLZ2",nObjective,control= list(referencePoint=NULL,crossoverProbability=1,mutationProbability=1,WFGScaling=TRUE,mutationDistribution=20,crossoverDistribution=30)){
  if(!is.na(stringr::str_match(fun,"DTLZ"))){
    if(!is.na(stringr::str_match(fun,"DTLZ1")))
      problemType <- 1
    else
      problemType <- 2
  }

  if(!is.na(stringr::str_match(fun,"WFG"))){
    problemType <- 3
  }
  ...
  if((problemType==3) && (WFGScaling==TRUE)){
    offspring <- nsga2R::boundedSBXover(t(parent),rep(0,chromosomeLength),seq(2,2*chromosomeLength,2),crossoverProbability,crossoverDistribution)
  }else{
    offspring <- nsga2R::boundedSBXover(t(population),rep(0,chromosomeLength),rep(1,chromosomeLength),crossoverProbability,crossoverDistribution)
  }
  ...
  for(offspringIndex in 1:populationSize){
    ...
    offspringObjectiveValue <- cbind(offspringObjectiveValue,EvaluateIndividual(offspring[,offspringIndex],fun,...))
  }
  ...
  populationSize <- maximumPopulationSize
  ...
  if(newPopulationSize>populationSize){
    ...
    distFromLine <- CalcDistRefLineToSolution(normalizedObjective,referencePoint)
  ...
  generation <- list(population, populationObjective)
  return(generation)
}
...
NSGA3.default <- function(population,fun,nObjective,control=list(),...){
  ...
  if(is.null(con$referencePoint)){
    con$referencePoint <- createWeightsSobol(nDim=nObjective,nPoints = 5*nObjective)
  }
  nRefPoint <- ncol(con$referencePoint)

  if(nObjective!= nrow(con$referencePoint)){
  ...
  if(newPopulationSize>populationSize){
    ...
    # calculate distances from reference lines
    distFromLine <- CalcDistRefLineToSolution(normalizedObjective,con$referencePoint)
